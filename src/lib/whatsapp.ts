...
export function readTracking(): Tracking {
  ...
  const STORAGE_KEY = "actum_tracking";
...
export function buildHeadline(answers: Answers): string {
  const valor = labelFor(STEPS.find((s) => s.id === "valor")!, answers.valor);
  const devedor = labelFor(STEPS.find((s) => s.id === "devedor")!, answers.devedor);
  if (!answers.valor || !answers.devedor) return "Novo lead pelo formulário do site.";
  return `Novo lead: precatório ${devedor.toLowerCase()}, valor ${valor}`;
}

/** Monta o texto da mensagem que o lead envia para a Actum. */
